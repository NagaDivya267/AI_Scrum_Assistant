...
# ── Retro Assistant ───────────────────────────────────────────────────────────
if page == "🎡 Retro Assistant":

    retro_tab1, retro_tab2, retro_tab3, retro_tab4 = st.tabs([
        "😊 Team Pulse",
        "📊 Sprint Data",
        "🎡 Retro Wheel",
        "🎯 Action Tracker"
    ...
    with retro_tab1:
        ...
        mood_word = st.text_input("One word to describe this sprint")

        if st.button("Save Mood"):
            if "mood_history" not in st.session_state:
                st.session_state["mood_history"] = []
            st.session_state["mood"] = mood
            st.session_state["mood_word"] = mood_word
            st.session_state["mood_history"].append({
                "mood": mood,
                "word": mood_word,
                "time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
            })
            st.success(f"Mood saved! {mood} — *{mood_word}*")
        ...
        if st.session_state.get("mood_history"):
            ...
            for entry in reversed(st.session_state["mood_history"]):
                st.markdown(
                    f"**{entry['mood']}** &nbsp; _{entry['word']}_ &nbsp; `{entry['time']}`"
                )
    ...
    # ── Tab 3: Retro Wheel ────────────────────────────────────────────────────
    with retro_tab3:
        st.header("🎡 Retro Wheel")
        st.markdown("Spin the wheel to pick a retrospective topic for the team to discuss.")
        ...
        with col_spin:
            if st.button("🎰 Spin the Wheel!", use_container_width=True):
                st.session_state["wheel_result"] = random.choice(retro_topics)

            st.markdown("**Topics on the wheel:**")
            for topic in retro_topics:
                st.markdown(f"- {topic}")

        with col_result:
            if st.session_state["wheel_result"]:
                st.markdown("### 🎯 Today's Retro Topic")
                st.markdown(
                    f"""
                    <div style="
                        background: linear-gradient(135deg, #00D4AA22, #6C63FF22);
                ...
                st.markdown("### Hit **Spin the Wheel** to get started!")

    # ── Tab 4: Action Tracker ─────────────────────────────────────────────────
    with retro_tab4:
        st.header("🎯 Action Tracker")

        if "retro_actions" not in st.session_state:
            st.session_state["retro_actions"] = []

        st.subheader("Add New Action Item")
        col_owner, col_desc, col_due = st.columns([1, 2, 1])
        with col_owner:
            new_owner = st.text_input("Owner", placeholder="Name")
        with col_desc:
            new_action = st.text_input("Action", placeholder="What needs to be done?")
        with col_due:
            new_due = st.date_input("Due Date", value=datetime.date.today() + datetime.timedelta(days=14))

        if st.button("➕ Add Action"):
            if new_action.strip():
                st.session_state["retro_actions"].append({
                    "owner": new_owner.strip() or "—",
        ...
        if not st.session_state["retro_actions"]:
            st.info("No action items yet. Add one above after your retro.")
        else:
            for idx, item in enumerate(st.session_state["retro_actions"]):
                col_chk, col_action, col_owner_d, col_due_d, col_del = st.columns([0.5, 3, 1.5, 1.2, 0.5])
                with col_chk:
                    done = st.checkbox("", value=item["done"], key=f"done_{idx}")
                    st.session_state["retro_actions"][idx]["done"] = done
                with col_action:
                    label = f"~~{item['action']}~~" if done else item["action"]
                    st.markdown(label)
                with col_owner_d:
                    st.markdown(item["owner"])
                with col_due_d:
                    st.markdown(f"`{item['due']}`")
                with col_del:
                    if st.button("🗑", key=f"del_{idx}", help="Delete this action"):
                        st.session_state["retro_actions"].pop(idx)
                        st.rerun()

            completed = sum(1 for a in st.session_state["retro_actions"] if a["done"])
            total_actions = len(st.session_state["retro_actions"])
            st.markdown(f"**Progress:** {completed}/{total_actions} completed")
